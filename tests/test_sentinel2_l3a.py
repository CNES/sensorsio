...
from sensorsio import mgrs, sentinel2_l3a


def get_sentinel2_l3a_folder() -> str:
    """
    ...
    return os.path.join(
        ...
        "sentinel2",
        "l3a",
        "SENTINEL2X_20200715-000000-000_L3A_T31TCJ_C_V2-1",
    )


@pytest.mark.requires_test_data
def test_sentinel2_instantiate_l3a():
    """
    Test sentinel2 class instantiation
    """
    s2 = sentinel2_l3a.Sentinel2L3A(get_sentinel2_l3a_folder())

    assert s2.product_dir == get_sentinel2_l3a_folder()
    assert (s2.product_name == "SENTINEL2X_20200715-000000-000_L3A_T31TCJ_C_V2-1")
    assert s2.date == datetime.datetime(2020, 7, 15, 00, 00, 00)
    assert s2.year == 2020
    assert s2.day_of_year == 197
    assert s2.tile == "31TCJ"
    assert s2.cloud_cover == 0
    assert s2.satellite == sentinel2_l3a.Sentinel2L3A.Satellite.S2X
...
class ReadAsNumpyParams:
    ...
    bands: List[sentinel2_l3a.Sentinel2L3A.Band] = field(
        default_factory=lambda: sentinel2_l3a.Sentinel2L3A.GROUP_10M)
    masks: List[sentinel2_l3a.Sentinel2L3A.Mask] = field(
        default_factory=lambda: sentinel2_l3a.Sentinel2L3A.ALL_MASKS)
    scale: float = 10000
...
@pytest.mark.parametrize(
    "parameters",
    [
        ReadAsNumpyParams(bounds=rio.coords.BoundingBox(300000.0, 4790220.0, 301000, 4792220)),
        # Use bounds to set output region, with 20m bands
        ReadAsNumpyParams(
            bands=sentinel2_l3a.Sentinel2L3A.GROUP_20M,
        ...
        ReadAsNumpyParams(
            bands=sentinel2_l3a.Sentinel2L3A.GROUP_10M + sentinel2_l3a.Sentinel2L3A.GROUP_20M,
            bounds=rio.coords.BoundingBox(300000.0, 4790220.0, 301000, 4792220),
        ),
...
def test_read_as_numpy_xarray(parameters: ReadAsNumpyParams):
    ...
    """
    s2_dataset = sentinel2_l3a.Sentinel2L3A(get_sentinel2_l3a_folder())

    # Read as numpy part
    bands_arr, mask_arr, xcoords, ycoords, crs = s2_dataset.read_as_numpy(**parameters.__dict__)
